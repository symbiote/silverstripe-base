<div id="PageComments_holder" class="typography">
	...
	<% if DeleteAllLink %>
		<p><a class="button delete" href="$DeleteAllLink"><% _t('PageCommentInterface.DELETEALLCOMMENTS','Delete all comments on this page') %></a></p>
	<% end_if %>
	<p>
		<a class="icon rss" href="$CommentRssLink"><% _t('RSSFEEDCOMMENTS', 'RSS feed for comments on this page') %></a>
		<a class="icon rss" href="PageComment/rss" title="<% _t('RSSVIEWALLCOMMENTS', 'View all Comments') %>"><% _t('RSSFEEDALLCOMMENTS', 'RSS feed for all comments') %></a>
	</p>
</div>
