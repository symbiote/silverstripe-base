<div id="forum-login">
	<% if CurrentMember %>
		<p>
			<% _t('LOGGEDINAS','You\'re logged in as') %> <% if CurrentMember.Nickname %>$CurrentMember.Nickname<% else %>Anonymous<% end_if %> | <a href="ForumMemberProfile/edit" title="<% _t('PROFILEEXPLICATION','Click here to edit your profile') %>"><% _t('PROFILE','Profile') %></a> | <a href="ForumMemberProfile/logout/" title="<% _t('LOGOUTEXPLICATION','Click here to log out') %>"><% _t('LOGOUT','Log Out') %></a></p>
	<% else %>
		<ul>
			<li><a href="$LoginURL" title="<% _t('LOGINEXPLICATION','Click here to login') %>"><% _t('LOGIN','Login') %></a></li>
